...
class CleanMyKeyboardInputFilter : public InputEventFilter
{
public:
    CleanMyKeyboardInputFilter()
        : InputEventFilter(InputFilterOrder::CleanMyKeyboard)
    {
    }

    bool keyboardKey(KeyboardKeyEvent *e) override
    {
        if (e->key == Qt::Key_F12 && e->modifiers == (Qt::MetaModifier | Qt::ShiftModifier)
            && e->state == KeyboardKeyState::Pressed) {
            m_active = !m_active;

            if (m_active) {
                qDebug() << "CleanMyKeyboard: Activated";
            } else {
                qDebug() << "CleanMyKeyboard: Deactivated";
            }

#if KWIN_BUILD_NOTIFICATIONS
            KNotification *notification = new KNotification("cleanmykeyboard");
            notification->setComponentName("kwin");
            if (m_active) {
                notification->setText("Keyboard is in cleaning mode; input will be blocked. Use Shift+Win+F12 to exit");
            } else {
                notification->setText("Keyboard is no longer in cleaning mode");
            }
...
void InputRedirection::setupInputFilters()
{
    ...
    m_cleanMyKeyboardFilter = std::make_unique<CleanMyKeyboardInputFilter>();
    installInputEventFilter(m_cleanMyKeyboardFilter.get());
